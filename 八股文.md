...
#### 粗排模型中基于精排排序结果的蒸馏训练总结

##### 1. 背景与问题定义

​	在搜索 / 推荐系统中，粗排（Pre-Ranking）作为精排（Ranking）的前置模块，其核心目标并非得到最终最优排序，而是：

**在极低计算成本约束下，快速筛选出一批“高潜力候选”，确保精排阶段不遗漏高质量结果。**

###### 1.1 **传统粗排训练方式**

粗排模型通常基于用户行为数据进行监督训练：

- 正样本：用户点击 / 下单 / 支付的 item
- 负样本：曝光但未发生行为的 item
...
1. **曝光偏置（Exposure Bias）**
   - 未曝光 ≠ 不相关
   - 曝光未点击 ≠ 不优质
2. **位置 / 策略偏置（Position & Policy Bias）**
   用户行为强依赖于历史精排策略与 UI 展示位置，而非纯相关性。
3. **长尾与冷启动问题**
   曝光稀疏的 item 几乎无法获得有效监督信号。

结果是：

> **粗排模型学到的往往是“历史系统偏好”，而非真正的 relevance。**
...
将精排模型视为一个能力更强、特征更丰富的 **Teacher**，通过其排序结果反向指导粗排（Student）训练：

> **利用精排模型的排序判断构造正负样本或排序约束，作为粗排训练的补充监督信号。**

这是一种典型的 **排序蒸馏（Ranking Distillation / Teacher–Student）** 思路。
...
###### 方式二：对式（Pairwise）构造（更常用）

对于同一 query：

- 若精排认为 score(A) > score(B)
- 构造训练对：(A > B)
...
- 直接以精排 Top-N 排序作为 soft label
- 优化 NDCG / ListNet / ListMLE 等目标
...
> **用模型的“理性判断”替代用户的“随机行为噪声”。**

###### 4.2 覆盖长尾与冷启动样本

只要精排模型能够基于内容、语义、商家特征判断 item 质量，即可为：
...
> **将精排复杂特征空间中的排序结论，压缩迁移到粗排可承载的模型中。**
...
- A. 用户真实行为样本
- B. 精排蒸馏样本（排序对 / Top-K vs Tail）

联合优化目标：

```
Loss = α · L_behavior + β · L_distill
```
...
- 仅使用精排 score gap 足够大的样本对
- 仅蒸馏 Top-N vs Bottom-M
- 过滤精排模型不确定区间

###### 6.3 评估指标

- Offline：Recall@K、NDCG、精排命中率
- Online：整体 CTR / GMV、精排效率、系统时延

------

##### 7. 总结

基于精排排序结果对粗排进行蒸馏，本质上是：

> **用精排模型的排序认知，修正和补充粗排训练中由用户行为带来的偏置，使粗排在计算受限条件下，更接近“精排偏好”的候选筛选器。**

该方法已成为大规模搜索与推荐系统中粗排训练的主流增强手段之一。
...
#### [李宏毅DL课程最新补充：深度学习的优化器 & 解释模型](https://www.bilibili.com/video/BV13x411v7US?spm_id_from=333.788.videopod.episodes&vd_source=f740ff72b448a270a8990e020d263b59&p=6)

#### [“随机梯度下降、牛顿法、动量法、Nesterov、AdaGrad、RMSprop、Adam”，打包理解对梯度下降法的优化](https://www.bilibili.com/video/BV1r64y1s7fU/?vd_source=f740ff72b448a270a8990e020d263b59)
...
1.大模型激活函数有哪些，根据时间发展结合论文将，是否有可训练参数，几种区别，激活函数的门控作用。2.dropout在llm作用，是否有应用。（结合了微调项目回答）3.编码策略：有哪些，rope原理，长度外推方法，插值细节，其他长度外推方法。4.llm的归一化函数有哪些，目前用哪些，是否有可训练参数，具体发挥什么作用？5.RHLF开放问题：与sft区别，适用什么场景，举例说明6.幻觉开放问题：什么是幻觉，如何解决7.rhlf常问 ppo dpo grpo区别，公式，还有哪些方法

作者：机尼台镁
链接：https://www.zhihu.com/question/647498812/answer/1952843726868164667
来源：知乎
