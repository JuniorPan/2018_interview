...
//设n个人围坐在一个圆桌周围，现在从第s个人开始报数，数到第m个人，让他出局；然后从出局的下一个人开始报数，数到第m个人，再让他出局
void Josephus(SqList L, int n, int s, int m)
{
	int i, j, k, temp;
	if (m == 0)
	{
	...
	{
		temp = L.elem[k];
		L.elem[k] = L.elem[n - k - 1];
		L.elem[n - k - 1] = L.elem[k];
	}
