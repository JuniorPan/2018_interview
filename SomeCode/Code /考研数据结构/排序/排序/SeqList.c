...
void Max_Heapify(SqList L, int start, int length) //调整保持堆序
{
	int largest; //用来记录左右孩子中较大的
	int l = 2 * start + 1; //左孩子
	int r = 2 * start + 2; //右孩子

	if (l < length && L.elem[l] > L.elem[start])  //寻找左右孩子中较大的数
		largest = l;
	else
		largest = start;

	if (r < length && L.elem[r] > L.elem[largest])
		largest = r;
	if (largest != start)
	{
		exchange(&L.elem[start], &L.elem[largest]);
		Max_Heapify(L, largest, length);
	}
}

void createHeap(SqList L) //建堆
{
	int i; 
...
//利用快速排序将最后一个元素放在正确的位置上
int Partition(SqList L, int low, int high)
{
	int i = low, j = high, temp = L.elem[high];

	while (i < j)
	{
		while (i < j && L.elem[i] < temp) i++;
		if (i < j)
			L.elem[j--] = L.elem[i];
		while (i < j && L.elem[j] >= temp) j--;
		if (i < j)
			L.elem[i++] = L.elem[j];
	}
...
// int Partition(SqList L, int low, int high)
// {
// 	int i = low, j = high, temp = L.elem[low];
// 	while (i < j)
// 	{
// 		while (i < j && L.elem[j] >= temp) j--;
// 		if (i < j)
...
//利用快速排序寻找第i小的数据
void QuickSort_2(SqList L, int low, int high, int i)
{
	if (low < high)
	{
		int pivot = Partition(L, low, high);
		if (pivot = i - 1)
			printf("%d  ", L.elem[pivot]);
		else if (pivot < i - 1)
			QuickSort_2(L, pivot + 1, high, i);
		else
			QuickSort_2(L, low, pivot - 1, i);
	}
}


//利用快速排序把整数放在负数前面
void Partition_2(SqList L, int low, int high)
{
